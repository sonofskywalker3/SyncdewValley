...
function Find-StartGameButton {
    ...
    # Fallback: find button via screenshot + pixel matching
    # UIAutomator returns null root for Xamarin apps (SMAPI Launcher)
    # Take a screenshot, scan for the button's gray band near the bottom
    try {
        ...
        if ($wmSize -match 'Physical size:\s*(\d+)x(\d+)') {
            ...
            # Capture screenshot and find button Y by scanning for the gray button band
            # The "Start Game" button is the lowest of 4 gray buttons in the bottom half
            $screenshotPath = Join-Path $SYNC_ROOT ".button_detect.png"
            & $ADB exec-out screencap -p > $screenshotPath 2>$null
            if (Test-Path $screenshotPath) {
                $img = [System.Drawing.Bitmap]::new($screenshotPath)
                $imgW = $img.Width
                $imgH = $img.Height
                $scaleX = $w / $imgW
                $scaleY = $h / $imgH

                # Scan bottom 40% of image at center column for gray button pixels (R,G,B ~100-140)
                $centerX = [math]::Floor($imgW / 2)
                $startY = [math]::Floor($imgH * 0.6)
                $lastButtonY = -1
                $inButton = $false
                $buttonTop = -1
                for ($y = $startY; $y -lt $imgH; $y++) {
                    $pixel = $img.GetPixel($centerX, $y)
                    $isGray = ($pixel.R -gt 80 -and $pixel.R -lt 160 -and
                               [math]::Abs($pixel.R - $pixel.G) -lt 20 -and
                               [math]::Abs($pixel.R - $pixel.B) -lt 20)
                    if ($isGray -and -not $inButton) {
                        $inButton = $true
                        $buttonTop = $y
                    } elseif (-not $isGray -and $inButton) {
                        $inButton = $false
                        $buttonHeight = $y - $buttonTop
                        if ($buttonHeight -gt 10) {
                            # This is a real button band, remember its center
                            $lastButtonY = [math]::Floor(($buttonTop + $y) / 2)
                        }
                    }
                }
                $img.Dispose()
                ...
                if ($lastButtonY -gt 0) {
                    $cx = [math]::Floor($w / 2)
                    $cy = [math]::Floor($lastButtonY * $scaleY)
                    Write-Dim "  (detected via screenshot: ${w}x${h}, button at y=$cy)"
                    return "$cx $cy"
                }
            }
        }
